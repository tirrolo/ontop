 ...
 * @author mariano
 */
public class SparqlAlgebraToDatalogTranslatorSesame {
	...
	public SparqlAlgebraToDatalogTranslatorSesame(UriTemplateMatcher templateMatcher) {
	...
	protected static org.slf4j.Logger log = LoggerFactory
			.getLogger(SparqlAlgebraToDatalogTranslatorSesame.class);
...
//	public void translate(List<Variable> vars, List<Var> vars,
//			DatalogProgram pr, int i, int[] varcount) {
//
//		if (triples.size() == 1) {
...
//			Function joinAtom = ofac.getFunction(join, leftAtom, rightAtom);
//
//			CQIE newrule = ofac.getCQIE(head, joinAtom);
//			pr.appendRule(newrule);
//
...
//	}
	...
	private Term getBooleanTerm(ValueExpr expr) {
		if (expr instanceof Var) {
			...
			if (expr instanceof Regex) { // sesame regex is Binary, Jena N-ary
				...
				Term term3 = (flags != null) ? getBooleanTerm(flags) : ofac
						.getConstantNULL();
			...
			// boolean functions
			return getFunctionBoolean(function, term1, term2);
	...
	private Function getConstantFunctionTerm(org.openrdf.query.algebra.ValueConstant expr) {
		...
		if (v instanceof LiteralImpl) {
			...
			else if ((type == XMLSchema.DATETIME) || type.equals(XMLSchema.DATETIME)) constantFunction = ofac.getFunction(ofac
	...
	private Function getFunctionBoolean(BinaryValueOperator expr, Term term1,
			Term term2) {
...
//	private Term getOtherFunctionTerm(NAryValueOperator expr) {
//		Function builtInFunction = null;
