...
import it.unibz.krdb.obda.owlrefplatform.core.abox.S_VirtualABoxMaterializer;
import it.unibz.krdb.obda.owlrefplatform.core.abox.S_VirtualABoxMaterializer.S_VirtualTriplePredicateIterator;
 ...
 */
public class S_QuestOWL extends OWLReasonerBase implements OBDAOWLReasoner, OWLQueryReasoner {
	...
	private S_Quest questInstance = new S_Quest();

	private static Logger log = LoggerFactory.getLogger(S_QuestOWL.class);

	private S_QuestConnection conn = null;

	private S_QuestOWLConnection owlconn = null;
	 ...
	 * Default constructor.
	 */
	public S_QuestOWL(OWLOntology rootOntology, OBDAModel obdaModel, OWLReasonerConfiguration configuration, BufferingMode bufferingMode,
		...
		try {
			...
			if (unfoldingMode.equals(QuestConstants.CLASSIC)) {
				...
				if (bObtainFromMappings) {
					...
					S_VirtualABoxMaterializer materializer = new S_VirtualABoxMaterializer(obdaModelForMaterialization);
					S_VirtualTriplePredicateIterator assertionIter = (S_VirtualTriplePredicateIterator) materializer.getAssertionIterator();
