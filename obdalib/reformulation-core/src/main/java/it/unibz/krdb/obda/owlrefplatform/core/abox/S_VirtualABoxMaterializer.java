...
import it.unibz.krdb.obda.owlrefplatform.core.S_Quest;
 ...
 * 
 */
public class S_VirtualABoxMaterializer {

	private OBDAModel model;

	private Map<OBDADataSource, S_Quest> questInstanceMap = new HashMap<OBDADataSource, S_Quest>();
	 ...
	 * @throws Exception
	 */
	public S_VirtualABoxMaterializer(OBDAModel model) throws Exception {
		this.model = model;
		...
		for (OBDADataSource source : model.getSources()) {
			...
			S_Quest questInstance = new S_Quest();
	...
	public Iterator<Assertion> getAssertionIterator() throws Exception {
		return new S_VirtualTriplePredicateIterator(vocabulary.iterator(), model.getSources(), questInstanceMap);
	}
	...
	public Iterator<Assertion> getAssertionIterator(Predicate p) throws Exception {
		return new S_VirtualTripleIterator(p, this.model.getSources(), questInstanceMap);
	}
	...
	public List<Assertion> getAssertionList(Predicate p) throws Exception {
		S_VirtualTripleIterator it = new S_VirtualTripleIterator(p, this.model.getSources(), questInstanceMap);
		List<Assertion> assertions = new LinkedList<Assertion>();
		while (it.hasNext()) {
			assertions.add(it.next());
		}
	 ...
	 */
	public class S_VirtualTriplePredicateIterator implements Iterator<Assertion> {

		private Iterator<Predicate> predicates;
		private Collection<OBDADataSource> sources;
		private Map<OBDADataSource,S_Quest> questInstances;

		private Predicate currentPredicate = null;
		private S_VirtualTripleIterator currentIterator = null;

		public S_VirtualTriplePredicateIterator(Iterator<Predicate> predicates, Collection<OBDADataSource> sources,
				Map<OBDADataSource, S_Quest> questInstances) throws SQLException {
		...
		private void advanceToNextPredicate() throws NoSuchElementException, SQLException {
			...
			currentIterator = new S_VirtualTripleIterator(currentPredicate, sources, questInstances);
		}
	 ...
	 * 
	 */
	public class S_VirtualTripleIterator implements Iterator<Assertion> {
		...
		private S_QuestConnection conn;
		private S_QuestStatement st;

		private Logger log = LoggerFactory.getLogger(S_VirtualTripleIterator.class);

		public S_VirtualTripleIterator(Predicate predicate, Collection<OBDADataSource> sources, Map<OBDADataSource, S_Quest> questInstances)
				throws SQLException {
		...
		private void advanceToNextSource() throws Exception {
			...
			S_Quest questInstance = questInstances.get(currentSource);
