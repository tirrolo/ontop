...
import it.unibz.krdb.obda.owlrefplatform.core.abox.S_RDBMSSIRepositoryManager;
...
import it.unibz.krdb.obda.owlrefplatform.core.mappingprocessing.S_TMappingProcessor;
...
import it.unibz.krdb.obda.owlrefplatform.core.tboxprocessing.S_EquivalenceTBoxOptimizer;
...
public class S_Quest implements Serializable, RepositoryChangedListener {
	/**
	 * 
	 */
	...
	protected S_RDBMSSIRepositoryManager dataRepository = null;
	...
	private Logger log = LoggerFactory.getLogger(S_Quest.class);
	...
	public void setupRepository() throws Exception {
		...
		if (bOptimizeEquivalences) {
			...
			// inputTBox.toString());
			S_EquivalenceTBoxOptimizer equiOptimizer = new S_EquivalenceTBoxOptimizer(
					inputTBox);
		...
		try {
			...
			if (unfoldingMode.equals(QuestConstants.CLASSIC)) {
				...
				if (dbType.equals(QuestConstants.SEMANTIC)) {
					dataRepository = new S_RDBMSSIRepositoryManager(
...
//			System.out.println("Original mapping size: " + unfoldingProgram.getRules().size());
			...
			S_TMappingProcessor tmappingProc = new S_TMappingProcessor(
...
//					+ (endTime - startTime) + " ms");
			...
			if (bOptimizeTBoxSigma) {
				S_SigmaTBoxOptimizer reducer = new S_SigmaTBoxOptimizer(
	...
	public S_QuestConnection getConnection() throws OBDAException {

		return new S_QuestConnection(this, getSQLConnection());
	}
