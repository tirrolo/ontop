...
public class MappingTreeModel extends DefaultTreeModel implements MappingControllerListener {
	...
	public MappingTreeModel(APIController apic, DatasourcesController dsc, MappingController controller) {
	...
	public void valueForPathChanged(TreePath path, Object newValue) {
		DefaultMutableTreeNode oldmappingnode = (DefaultMutableTreeNode) path.getLastPathComponent();
		DefaultMutableTreeNode parentnode = (DefaultMutableTreeNode)oldmappingnode.getParent();
...
//			DataSourceMapping mapping = controller.getMapping(source_uri, (String) oldmappingid);
//			mapping.setId((String) newValue);
//			controller.fireMappigUpdated(source_uri, mapping.getId(), mapping);
//			// controller.treeStructureChanged(e);
			controller.updateMapping(sourceName, (String)oldmappingvalue, (String)newValue);
		} else if (oldmappingnode instanceof MappingHeadNode) {
//			DefaultMutableTreeNode parent = (DefaultMutableTreeNode) oldmappingnode.getParent();
//			String mappingId = (String) parent.getUserObject();
//			DataSourceMapping mapping = controller.getMapping(source_uri, mappingId);
//			mapping.setTargetQuery(OntologyQuery.getFromString((String) newValue));
			ConjunctiveQuery newquery;
			try {
				newquery = new ConjunctiveQuery((String)newValue, apic);
				controller.updateMapping(sourceName, (String)parentnode.getUserObject(), newquery);
			} catch (QueryParseException e) {
				//If there was an error creating the new query, revert the changes
...
//			controller.fireMappigUpdated(source_uri, mapping.getId(), mapping);
			...
			try {
				...
				controller.updateMapping(sourceName, (String)parentnode.getUserObject(), query);
			} catch (QueryParseException e) {
				MappingBodyNode node = (MappingBodyNode)path.getLastPathComponent();
				OBDAMappingAxiom mapping = controller.getMapping(sourceName, (String)parentnode.getUserObject());
	...
	public void mappingInserted(String srcuri, String mapping_id) {
		...
		if ((currentsource == null)|| !srcuri.equals(currentsource.getName())) {
			return;
		}
		...
		// OF THIS EVENT
			...
			removeTreeModelListener(controller);
			insertNodeInto(mappingNode, (DefaultMutableTreeNode) root, root.getChildCount());
...
//			nodesWereInserted(mappingNode, indexesMapping);
	...
	public void currentSourceChanged(String oldsrcuri, String newsrcuri) {
		...
		try {
			...
			ArrayList<OBDAMappingAxiom> newmappings = controller.getMappings(newsrcuri);
			if (newmappings != null) {
